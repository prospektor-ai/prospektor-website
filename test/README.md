...
| `npm run resources:coverage` | Does `/resources/` still tell the truth about the product? Every screen, button or verb a surface declares in `names:` against the studio's string inventory (`data/studio-strings.json`, vendored by `npm run strings:snapshot`) and the help snapshot, and every link into `/help/` and `/resources/` against the pages that exist (#745, the website half of the studio's #744). `npm test` runs the same check. | Nothing (a studio checkout beside this repo, only to refresh the snapshot) |
