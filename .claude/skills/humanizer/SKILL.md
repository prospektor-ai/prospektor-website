---
name: humanizer
description: Rewrite AI-sounding text so it reads like a person wrote it, without changing what it says. Use on any sentence a visitor reads (a page, the scan field's copy, the checkout, a mail a function sends, a catalogue's translation) for AI tells - not-X-but-Y contrasts, one-line closers, staged openers, forced triads, dashes everywhere, inflated claims, sales language, stock AI words, bold labels, filler. Vendored from blader/humanizer (MIT) for #640; the same patterns gate `npm test` through lib/humanize.js.
license: MIT
metadata:
  version: "3.0.0"
...
<!--
  The website's copy of blader/humanizer SKILL.md v3.0.0 (MIT, LICENSE beside
  this file), unchanged below this note and word for word the studio's copy
  in prospektor-ai/studio (#634). Vendored here for #640, because
  prospektor.ai is written by hand by the same threads and had no check.
  Three things are ours:

  - `npm run humanize` lists every string on the pages, in the scripts, in
    the functions' mails and replies, and in each language's catalogue with
    a tell, and `npm test` is red on a strong one or on the dash count
    rising (tools/humanize.js holds the ceilings; lib/humanize.js the
    patterns, carried over from the studio unchanged so the two repos
    disagree about nothing).
  - There are no prompts here: nothing on this site is generated, so the
    standard is applied by the thread writing the copy. Read the seven tells
    in CLAUDE.md ("The voice contract") before writing a sentence, and this
    file when rewriting one.
  - The legal pages (/terms/, /privacy/, /dpa/) are COMPLIANCE's and carry
    sentences other tests pin by name; their dashes are measured and held,
    not rewritten. A translation keeps its own language's punctuation (the
    raya and the Gedankenstrich are not tells), the way #634 decided; the
    rule lives on the English key.
-->
