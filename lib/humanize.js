...
function htmlBlocks(html) {
  ...
  const text = h
    // A block's raw newlines are whitespace, the way a browser reads them.
    // Until #738 a line break inside a paragraph ended the string here, so
    // the scanner read markdown source lines: a not-X-but-Y that wrapped
    // (*is not X.* ⏎ *It is Y*) was never matched, a quoted phrase that
    // wrapped lost its exemption, and a paragraph was measured as its longest
    // line. Folded first, so every split below is on a block tag alone.
    .replace(/[\r\n]+/g, ' ')
    // A <blockquote> is somebody else's words. Each of its blocks is wrapped in
