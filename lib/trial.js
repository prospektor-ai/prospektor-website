...
// **Flipped true 11 Sep 2026 by #625**, on evidence rather than on a decision:
// a test-mode checkout on the `close-trial-proof` branch provisioned a real
// workspace and charged nothing (`paid: true, trial: true, amount_total: 0`),
// and Stripe's own page stated *30 days free · $0.00 due today · $999.00/month
// starting October 11*. Day 31 was not watched and could not be: this checkout
// sends `customer_email` and no `customer` id, and a Stripe test clock attaches
// to a customer — but the conversion runs `invoice.paid` -> resume, the same
// path every existing monthly renewal already takes in production.
const PROVEN = true;
